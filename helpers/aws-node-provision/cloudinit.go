...
lock() { flock -n /var/lock/node-bootstrap.lock "$@"; }
...
apt-get install -y ca-certificates curl gnupg apt-transport-https \
  wireguard wireguard-tools iputils-ping
...
for i in $(seq 1 12); do
  ip addr show wg0 2>/dev/null | grep -q 'inet ' && break
  sleep 5
done
...
# Retry join up to 5 times with exponential backoff
