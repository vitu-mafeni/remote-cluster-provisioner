...
net.ipv4.ip_forward=1" | sudo tee /etc/sysctl.d/k8s.conf`,
		...
		},
		{
			...
			cmds: []string{
				"sudo apt-get update",
				"sudo DEBIAN_FRONTEND=noninteractive apt-get install -y ca-certificates curl gnupg apt-transport-https cri-tools",
...
crictl --version || (echo "crictl installation failed"; exit 1)`, repoVersion),
		...
		| sudo tee /etc/apt/sources.list.d/cri-o.list > /dev/null`, repoVersion),
				"sudo apt-get update",
				"sudo DEBIAN_FRONTEND=noninteractive apt-get install -y jq criu crun conmon",
...
crun --version`,
				...
				"sudo DEBIAN_FRONTEND=noninteractive apt-get install -y cri-o",
...
| sudo tee /etc/crio/crio.conf.d/10-crun.conf`,
				...
				"sudo DEBIAN_FRONTEND=noninteractive apt-get install -y libcap2 libnl-3-200 libbsd0 libgnutls30",
